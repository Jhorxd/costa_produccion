...
<table class="full-width mt-10 mb-10">
    ...
    <tr>
        ...
        <th class="border-top-bottom desc-9 text-left">LOTE</th>
        <th class="border-top-bottom desc-9 text-left">F. VENC.</th>
    ...
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-center desc-9 align-top font-bold">@if($row->relation_item->date_of_due){{ $row->relation_item->date_of_due->format('Y-m-d')  }} @endif</td>
    ...
    @if($document->total_exportation > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">OP.
    ...
    @if($document->total_free > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">OP. GRATUITAS: {{ $document->currency_type->symbol }}</td>
            <td class="text-right font-bold desc">{{ number_format($document->total_free, 2) }}</td>
        </tr>
    @endif
    @if($document->total_unaffected > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">OP. INAFECTAS: {{ $document->currency_type->symbol }}</td>
    ...
    @elseif($document->total_taxed > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">OP. GRAVADAS: {{ $document->currency_type->symbol }}</td>
            <td class="text-right font-bold desc">{{ number_format($document->total_taxed, 2) }}</td>
        </tr>
    @endif
    ...
    @if($document->total_plastic_bag_taxes > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">ICBPER: {{ $document->currency_type->symbol }}</td>
            <td class="text-right font-bold desc">{{ number_format($document->total_plastic_bag_taxes, 2) }}</td>
        </tr>
    @endif
    <tr>
        <td colspan="7" class="text-right font-bold desc">IGV: {{ $document->currency_type->symbol }}</td>
        <td class="text-right font-bold desc">{{ number_format($document->total_igv, 2) }}</td>
    </tr>

    @if($document->total_isc > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">ISC: {{ $document->currency_type->symbol }}</td>
    ...
    @if($document->total_discount > 0 && $document->subtotal > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">SUBTOTAL: {{ $document->currency_type->symbol }}</td>
    ...
    @if($document->total_discount > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">DESCUENTO
    ...
    @if($document->total_charge > 0)
        @if($document->charges)
            ...
            @endphp
            <tr>
                <td colspan="7" class="text-right font-bold desc">CARGOS ({{$total_factor}}
                    %): {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_charge, 2) }}</td>
            </tr>
        @else
            <tr>
                <td colspan="7" class="text-right font-bold desc">CARGOS: {{ $document->currency_type->symbol }}</td>
                <td class="text-right font-bold desc">{{ number_format($document->total_charge, 2) }}</td>
            </tr>
        @endif
    @endif
    ...
    <tr>
        <td colspan="7" class="text-right font-bold desc">TOTAL A PAGAR: {{ $document->currency_type->symbol }}</td>
        <td class="text-right font-bold desc">{{ number_format($document->total, 2) }}</td>
    </tr>

    @if(($document->retention || $document->detraction) && $document->total_pending_payment > 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">M. PENDIENTE: {{ $document->currency_type->symbol }}</td>
    ...
    @if($balance < 0)
        <tr>
            <td colspan="7" class="text-right font-bold desc">VUELTO: {{ $document->currency_type->symbol }}</td>
