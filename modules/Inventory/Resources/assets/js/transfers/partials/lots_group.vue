...
export default {
    ...
    methods: {
        ...
        async submit() {
            // Validación opcional: al menos un lote con cantidad comprometida
            const selected = this.lots_group_.filter(x => x.compromise_quantity > 0)
            if (selected.length === 0) {
                return this.$message.warning('Debe comprometer al menos un lote.');
            }
